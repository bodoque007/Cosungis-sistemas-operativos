...
{
	//Obtenemos el numero de grupo de bloques donde se encuentra ese numero de inodo
	unsigned int numeroGrupoBloquesInodo = blockgroup_for_inode(inode_number);

	//Obtenemos el grupo de bloques donde se encuentra ese numero de inodo
   	Ext2FSBlockGroupDescriptor *  bloqueDescriptor = block_group(numeroGrupoBloquesInodo);

	// Obtenemos la posicion inicial de LBA  - tabla de inodos dentro del grupo
	unsigned int inodeTable =  bloqueDescriptor->inode_table; 

	// Obtenemos el offset del inodo dentro de la tabla
	unsigned int offsetTablaInodos = blockgroup_inode_index(inode_number);

	// Tamaño del bloque11
	unsigned int block_size = 1024 << _superblock->log_block_size;

	// Cantidad de inodos dentro de un bloque
	unsigned int cantidad_inodos_por_bloque = block_size / _superblock->inode_size;

	// LBA del bloque donde se encuentra el inodo
	unsigned int bloque = offsetTablaInodos / cantidad_inodos_por_bloque;

    Ext2FSInode* inodo = (Ext2FSInode*) malloc(sizeof(Ext2FSInode));
	unsigned char* buffer = (unsigned char*) malloc(block_size);

	read_block(inodeTable + bloque, buffer);
	
	unsigned int offset_inodo_bloque = offsetTablaInodos % cantidad_inodos_por_bloque;


	memcpy(inodo, buffer + (offset_inodo_bloque * _superblock->inode_size), sizeof(Ext2FSInode));
